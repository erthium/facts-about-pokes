...
export class PokeService {
    ...
    async getAllPokemonAbilities(name: string): Promise<string[]> {
        ...
        const ability_names: string[] = (await CsvService.getLineByIndex(AppConfig.poke_ability_data_path, 1)).split(',');
        const pokemon_abilities: string[] = (await CsvService.getLineByColumn(AppConfig.poke_ability_data_path, 0, name)).split(',');
